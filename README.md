# StockAI — A-share + US Stock Analysis Platform

基于 LangGraph 的 A 股 + 美股多智能体分析平台。A 股行情来自 AkShare/东方财富；美股行情来自 Yahoo Finance（Stooq 兜底）、财报来自官方 SEC EDGAR。基本面估值 Agent 基于近一年财报多方法估算合理股价区间，再由 LLM 生成中文分析报告，并提供多用户股票池、标签与 K 线图展示。
...
us_data -> us_fundamental -> us_validator
```

| 组件 | 端口 | 职责 |
|---|---:|---|
| Registry | 8001 | 智能体注册与心跳 |
| Orchestrator (A 股) | 8020 | A 股 LangGraph 编排、运行管理、SQLite checkpoint |
...
| US Orchestrator | 8029 | 美股独立编排进程（独立队列与 checkpoint，不与 A 股互相影响） |
| `us_data` | 8026 | 美股 OHLCV（Yahoo/Stooq）+ 公司信息，输出与 A 股同构 |
| `us_fundamental` | 8027 | SEC EDGAR 财报 + 配置/LLM 校验同行 + 多方法美元估值 |
| `us_validator` | 8028 | LLM 校验估值假设与数据质量（确定性规则兜底） |
...
`--symbols` 刷新指定股票；`--all` 刷新 watchlist 数据库中所有去重后的股票（跨全部用户，无论谁在关注），适合批量重跑。可选 `--market a|us`：`us` 会向美股编排器（默认 `http://127.0.0.1:8029`）提交并对美股 ticker 校验；`--db` 默认 `state/stock_portal.db`。要求 registry、各 agent 与对应 orchestrator 服务已启动；退出码 0 表示全部提交成功。
...
- 美股：门户内“A 股/美股”页签隔离股票池、标签与历史图；同一天刷新按市场所在时区判断。
